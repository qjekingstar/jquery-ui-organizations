+(function($,undefined){
	...
	$.widget("ui.organizations", {
		...
		_initNodes: function(){
			...
			rootHtml += '>'+this.options.rootTitle+'</span></li>';
			var root = $(rootHtml);
			var sRoot = $("<ol></ol>");
			root.append(sRoot);
			rootOL.append(root);
			var createNode = function(ele,nodeId,nodeObject){
				var node = $("<li></li>");
				...
				if(nodeObject.template){
					...
					span.html(nodeObject.data.title);
				}
				node.append(span);
				...
				if(children){
					for(id in children){
						createNode(childrenOL,id,children[id]);
					}
				}
			...
			for(id in this.options.nodes){
				createNode(sRoot,id,this.options.nodes[id]);
			}
			...
			this._repaint();
			this.element.children("ol").children("li").draggable().find("li").draggable({ 
				revert: "invalid",
			...
			this.element.find("span.panel").droppable(dropSettins);
		}
	});
})(jQuery);
